...
// Tableau de bord interne INDÉPENDANT de l'app, servi sur /cockpit, protégé par
// un mot de passe dédié (≠ comptes utilisateurs). En prod, tant que COCKPIT_PASSWORD
// n'est pas défini, le cockpit reste fermé (login renvoie 503). En local, mot de
// passe de dev par défaut pour pouvoir prévisualiser.
...
// ─── Centre de Commandement : coquille HTML (les données sont protégées par mot de passe côté API) ──
app.get('/cockpit', (req, res) => {
  res.set({
    'Cache-Control': 'no-store, no-cache, must-revalidate, max-age=0',
    'Content-Type':  'text/html; charset=utf-8'
  });
  res.sendFile(path.join(__dirname, 'public', 'cockpit', 'index.html'), { etag: false, lastModified: false });
});
...
// Login cockpit : compare au mot de passe dédié (comparaison à temps constant).
